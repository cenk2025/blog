...
export default async function AdminCategories() {
  ...
  // Fetch categories and a count of how many posts belong to each category
      ...
      <div className="overflow-hidden rounded-2xl border border-black/5 bg-white/50 dark:border-white/10 dark:bg-white/[0.02]">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-black/5 dark:divide-white/10">
            {categories?.map((cat) => (
              <tr key={cat.id} className="transition hover:bg-black/5 dark:hover:bg-white/[0.02]">
                <td className="px-6 py-4 font-medium text-ink-light dark:text-ink-dark">
                  {cat.slug}
                </td>
                <td className="px-6 py-4">
                  {cat.translations?.en?.name || "Unnamed"}
                </td>
